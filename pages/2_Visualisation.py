...
if indice == "High vol PER":
    ...
    for year in range(2018, 2022):  
        interval = [year, year]
        for i in range(2):
            df_prices_months[f"df_prices_{year}_{i}"] = f.get_prices_series_2(df_prices, interval)[i]
    ...
    for year in range(2018, 2021):
        for i in range(2):
            df_top_100[f"df_top_100_{year}_{i}"] = f.selection_top_100(df_returns_months[f"df_returns_{year}_{i}"])
    ...
    for year in range(2018, 2021):
        df_capitalization_countries_years[f"df_capitalization_countries_{year}"] = f.get_indicator_data_num(df_qualitative_years[f"df_qualitative_{year}"], indicator)
    ...
    title = f"Évolution de l'indice {country} entre 2018 et 2021"
